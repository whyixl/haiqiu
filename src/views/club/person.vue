    ...
    <el-card :body-style="{ padding: '0px' }" shadow="never">
      ...
      <el-table :data="pager.records" @selection-change="onSelectionChange" highlight-current-row stripe
        ...
        <el-table-column align="center" label="惯用脚" prop="personDetail">
          <template slot-scope="scope">
            {{!scope.row.personDetail? '--' : (scope.row.personDetail.preferredSide=="left_foot" ? '左脚' :
            scope.row.personDetail.preferredSide=="right_foot" ? '右脚'
            :scope.row.personDetail.preferredSide=="double_foot" ? '双脚' :'--')}}
          </template>
        </el-table-column>
        ...
        methods: {
            ...
            remove(id, flag) {
                this.$http.get('/teamPerson/selectByPerson', {
                    params: {size: 1, current: 1, peId: id}
                }).then(res => {
                    ...
                    if (flag && (!records || records.length === 0)) {
                        ...
                        this.$message({
                            message: "目前该人员存在某些历史记录，删除会导致系统错误",
                            showClose: true,
                            type: 'error'
                        })
                    }
